...
CREATE TABLE instructor_face (
                                 ...
                                 face_image_url TEXT,             -- ADDED: This was missing but used in your function
                                 luxand_subject_id TEXT,          -- RENAMED: Replaced person_id_azure
                                 date_created TIMESTAMP NOT NULL DEFAULT now(),
                                 created_by TEXT NOT NULL,
...
-- FACE RECOGNITION CRUD FUNCTIONS (Updated for Luxand)
CREATE OR REPLACE FUNCTION instructor_face_create(
    p_ins_id BIGINT,
    p_face_uuid TEXT,
    p_face_image_url TEXT,
    p_luxand_subject_id TEXT, -- ADDED
    p_created_by TEXT
) RETURNS instructor_face AS $$
DECLARE
_f instructor_face%ROWTYPE;
BEGIN
INSERT INTO instructor_face(ins_id, face_uuid, face_image_url, luxand_subject_id, created_by)
VALUES (p_ins_id, p_face_uuid, p_face_image_url, p_luxand_subject_id, p_created_by)
    RETURNING * INTO _f;
RETURN _f;
END;
$$ LANGUAGE plpgsql;
