...
const InstructorList = () => {
    ...
    // --- *** NEW STATE ADDED *** ---
    const [instructorFaces, setInstructorFaces] = useState([]);
    ...
    const instructorsWithSubjects = useMemo(() => {
        return instructors.map(instructor => {
            ...
            // --- *** START OF CHANGES *** ---
            // 1. Find the active face record for this instructor
            const faceRecord = instructorFaces.find(
                f => f.ins_id === instructor.ins_id && f.is_active
            );

            // 2. Use the URL if it exists, otherwise use the default
            const faceUrl = faceRecord ? faceRecord.face_image_url : "/profiles/profile-default.png";
            // --- *** END OF CHANGES *** ---
        ...
        // --- *** ADDED instructorFaces TO DEPENDENCY ARRAY *** ---
    }, [instructors, subjects, instructorSubjects, instructorFaces]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // --- *** START OF CHANGES *** ---
                // Fetch all required data, including the new 'instructor-faces'
                const [
                    instructorsRes,
                    subjectsRes,
                    ...
                    studentSectionsRes,
                    instructorFacesRes // 1. Added new variable
                ] = await Promise.all([
                    ...
                    fetch("/instructor-faces/all-faces").then(res => res.json()) // 2. Added new fetch call
                ]);
                ...
                const responses = [
                    ...
                    { data: instructorFacesRes, name: "instructor-faces" } // 3. Added to error check
                ];
                // --- *** END OF CHANGES *** ---
                ...
                setSectionAssignments(sectionAssignmentsRes);
                setStudentSections(studentSectionsRes);
                // --- *** NEW STATE SET *** ---
                setInstructorFaces(instructorFacesRes.faces); // 4. Set the new state
    ...
    // Handle instructor selection and subject expansion (Unchanged)
    useEffect(() => {
    ...
    // Filter instructors based on search and filters (Unchanged)
    ...
    // Get subjects to show in modal (Unchanged)
    ...
    // Get students by section using student-sections relationships (Unchanged)
    ...
    // Find sections for instructor and subject using section-assignments (Unchanged)
    ...
    // ... (handleOpenModal, handleCloseModal, handleSubjectToggle, loading/error return, main JSX)
    // ...
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex flex-col">
            ...
            <main className="flex-1 p-6 max-w-7xl mx-auto w-full">
                ...
                {/* Instructors Grid Section (Unchanged, will now work) */}
                ...
                {/* Modal / Popup (Unchanged, will now work) */}
                {selectedInstructor && (
