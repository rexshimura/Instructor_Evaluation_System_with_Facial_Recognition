...
export default function AdminCurriculum() {
  const [subjects, setSubjects] = useState(subjectLoad);
  ...
  useEffect(() => {
    if (subjectId) {
      const subject = subjects.find(s => s.sb_subID === subjectId);
      if (subject) {
  ...
  const coursePrefixes = useMemo(() => {
    ...
    const prefixes = subjects.filter(s => s.sb_course === filters.course).map(s => s.sb_miscode.replace(/[0-9]/g, ''));
  ...
  const handleUpdateSubject = (updatedSubject) => {
    setSubjects(subjects.map(s => s.sb_subID === updatedSubject.sb_subID ? updatedSubject : s));
    ...
    <div className="h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 flex flex-col p-4 md:p-8 gap-6 overflow-hidden">
        ...
        <div className="flex justify-between items-center">
          ...
          <button onClick={() => setIsAddModalOpen(true)} className="flex items-center gap-2 bg-green-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-700 transition duration-150 shadow-md">
           ...
           <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                <div>
                    ...
                    <select name="course" value={filters.course} onChange={handleFilterChange} className="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                        <option value="All">All Courses</option><option value="BSIT">BSIT</option><option value="BSIS">BSIS</option>
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="year" value={filters.year} onChange={handleFilterChange} className="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                        <option value="All">All Years</option>{[1, 2, 3, 4].map(y => <option key={y} value={y}>{y}</option>)}
                    </select>
                </div>
                <div>
                    ...
                    <select name="semester" value={filters.semester} onChange={handleFilterChange} className="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                        <option value="All">All Semesters</option>{[1, 2].map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                </div>
                <div>
                    ...
                    <select name="units" value={filters.units} onChange={handleFilterChange} className="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                        <option value="All">All Units</option>{[1, 2, 3, 6].map(u => <option key={u} value={u}>{u}</option>)}
                    </select>
                </div>
            </div>
        </div>
          ...
          <div className="overflow-auto flex-1">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-100 sticky top-0 z-10">
                  <tr>{["MIS Code", "Subject Name", "Course", "Year", "Sem", "Units", ""].map(header =>
                    <th key={header} scope="col" className="px-6 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider bg-gray-100">{header}</th>
                  )}</tr>
              </thead>
                ...
                {filteredSubjects.length > 0 ? (
                  filteredSubjects.map((subject) => (
                    <tr key={subject.sb_subID} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 max-w-xs truncate" title={subject.sb_name}>{subject.sb_name}</td>
                      <td className="px-6 py-4 whitespace-nowrap"><span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${ subject.sb_course === 'BSIT' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800' }`}>{subject.sb_course}</span></td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center justify-end space-x-2">
                            <button onClick={() => handleOpenEditModal(subject)} className="p-2 text-indigo-600 bg-indigo-100 rounded-full hover:bg-indigo-200 transition-colors" title="Edit"><FiEdit size={16}/></button>
                            <button onClick={() => handleDeleteSubject(subject.sb_subID, subject.sb_name)} className="p-2 text-red-600 bg-red-100 rounded-full hover:bg-red-200 transition-colors" title="Delete"><FiTrash2 size={16}/></button>
                        </div>
                      </td>
                    </tr>
                  ))
      ...
      {/* Modals */}
      <AddSubjectModal show={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onSave={handleAddSubject} />
      {subjectToEdit && <EditSubjectModal show={isEditModalOpen} onClose={handleCloseEditModal} subject={subjectToEdit} onSave={handleUpdateSubject} />}

      <footer className="bg-white text-center py-4 border-t border-gray-200"><p className="text-sm text-gray-600">© {new Date().getFullYear()} Faculty Profiling & Evaluation System (ProEv)</p></footer>
    </div>
  );
}
